...
diffRegression <- function(design.matrix.df, regression.type = "binomial", fast.reg = FALSE, dof = 0) {
  ...
  if (nrow(coeffs) < 2) {
    ...
    pval.att <- 2 * pt(abs(beta_zscore_a), res_df, lower.tail = FALSE) # two-sided p-val
  }
...
#' @export
#'
npdr_revised <- function(time, outcome, attr.mat,
  ...
  if (length(outcome) == 1) {
    ...
    time.vec <- time # get time
    outcome.vec <- outcome # get outcome
    attr.mat <- attr.mat # outcome = "qtrait" or 101
  } else { # user specifies a separate phenotype vector
    time.vec <- time # get time
    outcome.vec <- outcome # get outcome
    attr.mat <- as.matrix(attr.mat) # assumes dataset only contains attributes/predictors
  }
  ...
  min.class.size <- min(as.numeric(table(time.vec)))
  balanced.theoretical.k <- sNPDR::knnSURF(2*min.class.size - 1, 0.5)
  ...
  Ri.pheno.vals <- data.frame(time = time.vec[neighbor.pairs.idx[, 1]], 
                            outcome = outcome.vec[neighbor.pairs.idx[, 1]])

  NN.pheno.vals <- data.frame(time = time.vec[neighbor.pairs.idx[, 2]], 
                              outcome = outcome.vec[neighbor.pairs.idx[, 2]])

  # ----- change this to hit/miss using filtering -------------#
  if (regression.type == "lm") {
    # returns just the numeric difference of survival times
    pheno.diff.vec <- npdrDiff(Ri.pheno.vals$time, NN.pheno.vals$time, diff.type = "numeric-abs")
    ...
    # subset nearest neighbor and Ri vectors based on filtering
    NN.pheno.vals <- NN.pheno.vals[filtered_indices, ]
    Ri.pheno.vals <- Ri.pheno.vals[filtered_indices, ]
    ...
    # Reclassification according to lorenzo et. al. (see truth table)
    #NN.pheno.vals.mutate <- NN.pheno.vals %>%
    #    mutate(outcome = if_else(time > Ri.pheno.vals$time, FALSE, outcome),
    #          time = if_else(time > Ri.pheno.vals$time, Ri.pheno.vals$time, time))
    NN.pheno.vals.mutate <- NN.pheno.vals

    # pheno.diff.vec is the hit/miss vector so it will need to be based on the reclassified (mutated) nearest neighbors
    pheno.diff.vec <- npdrDiff(Ri.pheno.vals$outcome, NN.pheno.vals.mutate$outcome, diff.type = "match-mismatch") %>%
      as.factor()
    # since time is changed under the mutation, we will need to calculate the time diff vec based on the orginal nearest neighbor time
    time.diff.vec <- npdrDiff(Ri.pheno.vals$time, NN.pheno.vals$time, diff.type = "numeric-abs")
  }
  ...
  attr.diff.mat <- matrix(0, nrow = length(filtered_indices), ncol = num.attr)
  ...
  if (length(covars) > 1) { # if covars is a vector or matrix
    ...
    covar.diff.df <- data.frame(matrix(NA, nrow = nrow(neighbor.pairs.idx), ncol = 0))
  ...
  if (!use.glmnet) { # combine non-glmnet result lists into a matrix
      ...
      npdr.stats.df <- data.frame(npdr.stats.pval_ordered.mat)
       ...
       for (attr.idx in seq(1, num.attr)) {
        if (attr.diff.type == "correlation-data") { # corrdata
          ...
          # Filter Ri.attr.vals and NN.attr.vals using the captured indices
          Ri.attr.vals <- Ri.attr.vals[filtered_indices, ]
          NN.attr.vals <- NN.attr.vals[filtered_indices, ]
        } else {
          attr.vals <- attr.mat[, attr.idx]
          Ri.attr.vals <- attr.vals[neighbor.pairs.idx[, 1]]
          NN.attr.vals <- attr.vals[neighbor.pairs.idx[, 2]]
        ...
        #------- Before performing regression, transform distances by the Kaplan-Meier Estimate ---------
        if (KM.weight == TRUE){
          if (KM.kernal.type == 'linear') {
             # inner product of the two KM estimates
            KM.kernal <- Ri.KM.est$surv_prob * NN.KM.est$surv_prob
          }
          if (KM.kernal.type == 'gaussian') {
            # inner product of the two KM estimates
            KM.kernal <- exp((Ri.KM.est$surv_prob - NN.KM.est$surv_prob)^2 / (2 * KM.kernal.sigma^2))
          }
          design.matrix.df$attr.diff.vec <- design.matrix.df$attr.diff.vec * KM.kernal
        } else {
          design.matrix.df$attr.diff.vec <- design.matrix.df$attr.diff.vec
        }
    ...
    for (attr.idx in seq(1, num.attr)) {
      if (attr.diff.type == "correlation-data") { # corrdata
        ...
        # ---------- Transform the attribute space with the KM estimate----------#
      ...
      if(KM.weight == TRUE) {
        ...
        attr.diff.vec <- npdrDiff(Ri.attr.vals, NN.attr.vals, diff.type = attr.diff.type)
        attr.diff.vec <- attr.diff.vec * KM.kernal
      } else {
        attr.diff.vec <- npdrDiff(Ri.attr.vals, NN.attr.vals, diff.type = attr.diff.type)
      }
      attr.diff.mat[, attr.idx] <- attr.diff.vec
    ...
    # Reclassify nearest neighbors based on Ri (Lorenzo et al.)
    #NN.pheno.vals.mutate <- NN.pheno.vals %>%
    #mutate(outcome = if_else(time > Ri.pheno.vals$time, FALSE, outcome),
    #      time = if_else(time > Ri.pheno.vals$time, Ri.pheno.vals$time, time))
    NN.pheno.vals.mutate <- NN.pheno.vals
    # Discard category 4 instances
    NN.pheno.vals.mutate <- NN.pheno.vals.mutate[filtered_indices, ]

    if (glmnet.alpha != "cluster") { # temporary trick to cluster attributes by diff vector
      if (regression.type == "binomial") {
        pheno.diff.vec <- npdrDiff(Ri.pheno.vals, NN.pheno.vals, diff.type = "binomial-surv")
      ...
      if (regression.type == "binomial") {
        pheno.diff.vec <- npdrDiff(Ri.pheno.vals$outcome, NN.pheno.vals$outcome, diff.type = "match-mismatch")
      } else if (regression.type == "cox") {
        # Only keep the filtered instances
        NN.pheno.vals <- NN.pheno.vals[filtered_indices, ]
        Ri.pheno.vals <- Ri.pheno.vals[filtered_indices, ]
